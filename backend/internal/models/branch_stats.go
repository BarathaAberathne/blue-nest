...
// PerfDimension is one weighted input to the Branch Health score.
type PerfDimension struct {
	Label  string `json:"label"`
	Score  int    `json:"score"`  // 0–100
	Weight int    `json:"weight"` // percentage weight
}

// BranchPerformance is the weighted composite health score + its breakdown.
type BranchPerformance struct {
	Overall    int             `json:"overall"`
	Dimensions []PerfDimension `json:"dimensions"`
}
...
type BranchDashboard struct {
	...
	Ofsted               string               `json:"ofsted,omitempty"`
	Performance          int                  `json:"performance"`
	PerformanceBreakdown BranchPerformance    `json:"performance_breakdown"`
