...
export const api = {
  ...
  // Minimal assignable-role list for pickers (staff.manage, incl. custom roles).
  adminGetAssignableRoles: (token: string) => apiFetch<AssignableRole[]>("/api/v1/admin/roles/assignable", { token }),
  ...
  // Payroll roll-up (Phase D): per-staff worked hours + leave taxonomy for a
  // period, from the attendance register. Export lives at /admin/payroll/export.
  adminGetPayroll: (token: string, params: { from: string; to: string; branch?: string }) => {
    const qs = new URLSearchParams({ from: params.from, to: params.to });
    if (params.branch) qs.set("branch", params.branch);
    return apiFetch<PayrollSummary>(`/api/v1/admin/payroll?${qs}`, { token });
  },
  ...
  // Org-wide leadership directory (all branches) for the Leadership tab —
  // deliberately not branch-scoped, minimal projection only.
  adminGetLeadershipCandidates: (token: string) =>
    apiFetch<LeadershipCandidate[]>("/api/v1/admin/branches/leadership-candidates", { token }),
