...
And Assert pendingOrder.body.data.status == "pending"
And Assert pendingOrder.body.data.payment_status == "unpaid"

When Get /api/v1/admin/orders Into adminList Using adminSession.accessToken
Then AssertStatus adminList 200
And AssertJson adminList "$.body.data[?(@.id=='${checkout.body.data.order_id}')].length()" == 0
```
