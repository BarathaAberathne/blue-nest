...
| RoleSuite.tc_role_003_roleChangeRevokesActiveSessions | TC-ROLE-003 | `ROLE-TC-009` | ✅ completed | **policy inverted** — the auth-hardening pass (audit item 5) made role changes revoke existing tokens, so `ROLE-TC-009` locks the NEW behaviour (401 on the old token). The legacy test originally asserted the OLD behaviour and was left failing as "superseded"; that made every full legacy run permanently red (masking real regressions), so it was re-aligned (2026-09-11) to assert the same revocation policy — both platforms now lock the same behaviour and `make test-e2e` is green again |
